...
description: Detect and flag AI-generated low-quality or misleading content (slop) with comprehensive pattern catalog and tracking
version: "2.0.0"
...
Check for 46+ AI slop patterns across categories:
...
<step name="track">
Add detected slop to the tracker database (~/.hermes/slop-tracker/slop-log.json):
- Record: file path, line number(s), slop type/category, timestamp, status (pending/reviewed/removed)
- Add evidence: source line citation, pattern ID (V1, T4, C2, etc.), confidence level
- Maintain: versioned log with entries array, each entry has: id, file, line, pattern, confidence, severity, evidence, action_taken, resolved_at
...
- List all entries with: ID, file path, pattern ID, confidence, severity, status
- Group by: confidence level (Confirmed/Probable/Candidate), severity (Critical/High/Medium/Low)
...
1. **Broken behavior, accessibility, overflow, and readability** (Q1-Q8, C4, L6-L8, M2)
...
4. **Type hierarchy, spacing rhythm, and color roles** (T1-T10, C1-C5, L3, L5)
5. **Decorative borders, gradients, glows, radii, icons, and motion** (V1-V7, M1, M3)
...
- [ ] Track detected items in slop-log.json with full metadata (file, line, pattern, confidence, severity, evidence)
- [ ] Review tracked items with status updates and exemption tracking
- [ ] Remediate following prioritized fix order (6 stages)
- [ ] Maintain persistent slop tracking across sessions via slop-log.json
- [ ] Generate standardized report format with direction/findings/needs verification/exemptions/validation
- [ ] Integrate 46+ catalogued slop patterns across visual, typography, color, layout, motion, copy, imagery, quality
